...
const OrderDetails: React.FC<{ id: string }> = ({ id }) => {
  ...
  // Backend returns { ok: true, data: order }. This is a single object (not
  // an array), so the axios interceptor in client.ts leaves it untouched —
  // unwrap .data explicitly here rather than the nonexistent .order key.
  ...
  return (
    <Card>
      <Descriptions title={`Order ${order.id?.slice(0, 8).toUpperCase()}`} bordered>
        <Descriptions.Item label="Order No.">{order.id?.slice(0, 8).toUpperCase()}</Descriptions.Item>
        <Descriptions.Item label="Order Number (your reference)">{order.externalId || "—"}</Descriptions.Item>
